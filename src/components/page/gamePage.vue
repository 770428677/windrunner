<template>
  <div class="gamePage">
    <div class="video">
      ...
      <ul>
        <li v-for="(videoArr,index) in videoArr" :key="index">
          <ol>
            <li>
              <div class="articleContent">
                <video :src="videoArr.src" controls :poster="videoArr.poster">您的浏览器不支持 video 标签。请您更换其他浏览器</video>
                <h5>{{videoArr.name}}</h5>
              </div>
            </li>
          </ol>
    ...
    <div class="picture">
      ...
      <div class="waterFall-box" ref="box">
        <div class="img-box one_id" v-for="(item, i) in images" :key="i" ref="img">
          <img :src="item.src" alt="">
          <span>{{item.name}}</span>
        </div>
        <div v-if="isLoad === false"
                :style="{position: 'absolute', top: Math.max(...heightArray)+'px', padding: '1em', color: 'red', left: '50%', transform: 'translateX(-50%)'}">
          没有图片加载了...
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      videoArr: [
        {name: '带你领略魔兽中的风景', src: '/windrunner/static/video/fengjing.mp4', poster: '/windrunner/static/image/fe.jpg'},
        {name: '吾名死亡之翼，天命之灭世者，万物的终结者，无可阻挡，无可违逆，吾即大灾变！', src: '/windrunner/static/video/CTMcg.mp4', poster: '/windrunner/static/image/siwangzhiyi.jpg'},
        {name: '生生不息的仇恨，一再破裂的盟约，部落与联盟大战在即', src: '/windrunner/static/video/zhengba.mp4', poster: '/windrunner/static/image/zhengbaaizelasi.jpg'}
      ],
      images: [], // 存储图片资源
      imgWidth: 250, // 图片的宽度
  ...
  methods: {
    ...
    positionImg (start) {
      ...
      for (let i = start; i < this.images.length; i++) {
        ...
        // 获得当前图片的高度
        const currHeight = parentDom[i].clientHeight + 20
        // 定位
  ...
  mounted () {
    ...
    window.addEventListener('scroll', function () {
      ...
      if (scrollDistance === scroll) {
        Axios({
          url: '/windrunner/static/waterFall2.json',
          method: 'get'
        }).then(res => {
          if (res.status === 200) {
            _this.count += 1
            ...
            if (_this.isLoad) {
              ...
              for (let item of res.data.sites) {
  ...
  created () {
    Axios({
      url: '/windrunner/static/waterFall.json',
      method: 'get'
    }).then(res => {
      if (res.status === 200) {
        this.images = res.data.sites ? res.data.sites : []
        this.loadImgHeight()
      }
    })
  }
...
<style scoped>
  ...
  h4{
    ...
    background: url("../../../static/image/biaoti.jpg");
  }
  .gamePage{
    margin:1em auto;
    width:93vw;
